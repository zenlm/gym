...
def run_grpo(
    ...
    with training_args.main_process_first(desc="load dataset"):
        dataset = get_dataset(template, model_args, data_args, training_args, "grpo", tokenizer)
    ...
    model = load_model(
        ...
        add_value_head=False  # GRPO doesn't use value head
    )
    ...
    if training_args.do_train:
        logger.info_rank0("*** Starting GRPO training ***")
        logger.info_rank0(f"  Num examples = {len(train_dataset):,}")
        logger.info_rank0(f"  Num epochs = {training_args.num_train_epochs}")
        logger.info_rank0(f"  Group size = {finetuning_args.grpo_group_size}")
        logger.info_rank0(f"  Beta = {finetuning_args.grpo_beta}")
        logger.info_rank0(f"  Clip range = {finetuning_args.grpo_clip_range}")
    ...
    if training_args.push_to_hub:
        kwargs = {
            ...
            "tags": ["grpo", "reinforcement-learning", "gym"],
        }
